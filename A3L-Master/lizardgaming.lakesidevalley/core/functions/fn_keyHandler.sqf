...
switch (_code) do
{
	...
	case _interactionKey:
	{
	    ...
	    //Shift+N, Panic Button - Dizy (way more awkward then Jamal's keys) < -- No one cares! - Jamal
        
    case 49: {
    if (_shift && !_alt && !_ctrlKey) then {
        if (playerSide != west) exitWith {};
        
		[[[1],format["#### WARNING ####\n\n\n****\n\n\n%1 has activated their PANIC Button\n\n\n****\n\n\n#### WARNING ####",profileName]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
        _handled = true;
		sleep 10;
	...
	case 19:
	{
		...
		{
			player playmovenow "AinvPercMstpSnonWnonDnon_Putdown_AmovPercMstpSnonWnonDnon"; // added by heisen
	...
	//Shift+J Or Surrender... Jamal Macklemore (#Awkward Key) < -- You're telling me - Huskies xD
	case 36: {
		if (_shift && !_alt && !_ctrlKey) then {
	...
	//Radar (Another Shitty Awkward Key So Deal With It)
	case 48: {
		if (_shift && _alt && _ctrlKey) then {
			[] call life_fnc_abc12345;
		};
