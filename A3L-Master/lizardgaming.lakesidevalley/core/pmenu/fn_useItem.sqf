...
switch (true) do
{
	...
	case (_item == "Dawsey's Beer"):
	{
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
			life_thirst = 100;
			player setFatigue 0;
			hint "You Have Drunk One Of Dawsey's Best Beers!";
			[] spawn life_fnc_drinkthebeer;
		};
	};

	case (_item == "Jamal's Gin"):
	{
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
			life_thirst = 100;
			player setFatigue 0;
			hint "You Have Drunk One Of Jamal's Finest Gins!";
			[] spawn life_fnc_drinkthegin;
		};
	};

	case (_item == "Connor's Vodka"):
	{
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
			life_thirst = 100;
			player setFatigue 0;
			hint "You Have Drunk One Of Connor's Rocking Vodkas!";
			[] spawn life_fnc_drinkthevodka;
		};
	};

	case (_item == "Wild Whiskey"):
	{
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
			life_thirst = 100;
			player setFatigue 0;
			hint "You Have Drunk One Of Wildest Whiskeys Around!";
			[] spawn life_fnc_drinkthewhiskey;
		};
	};

	case (_item == "Harry's Ale"):
	{
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
			life_thirst = 100;
			player setFatigue 0;
			hint "You Have Drunk One Of Harry's Home-Brewed Specials!";
			[] spawn life_fnc_drinktheale;
		};
	};
	
	case (_item == "Zipties"):
	{
		[] spawn life_fnc_chainPlayerAction;
	};
	
	case (_item == "camonet"): {
		[] spawn life_fnc_drugfieldTent;
	};
	
	case (_item == "seedcan"): {
		[] spawn life_fnc_drugfieldcannabisSeed;
	};
	
	case (_item == "seedcoc"): {
		[] spawn life_fnc_drugfieldcocaSeed;
	};
	
	case (_item == "seedopi"): {
		[] spawn life_fnc_drugfieldopiumSeed;
	};
	
	case (_item == "shovel"): {
		[] spawn life_fnc_drugfieldShovel;
	};
	
	case (_item == "fertilizer"): {
		[] spawn life_fnc_drugfieldFertilizer;
	};
	
	case (_item == "generator"): {
		[] spawn life_fnc_drugfieldGenerator;
	};
	
	case (_item == "lightdouble"): {
		[] spawn life_fnc_drugfieldLight;
	};
