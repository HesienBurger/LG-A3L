...
switch (playerSide) do
{
	case civilian:
	{
		...
		//Unchain (Kinky I Know)
		life_actions = life_actions + [player addAction["Untie",life_fnc_unchainPlayerAction,cursorTarget,0,false,false,"",
		' !isNull cursorTarget && !(player getVariable "chained") && !(player getVariable "restrained") && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && animationState cursorTarget == "AmovPercMstpSnonWnonDnon_Ease" && player distance cursorTarget < 3.5 && (cursorTarget getVariable "chained") && !(cursorTarget getVariable "Escorting") ']];
	};
	case west:
	{
	// Heli Fast Rope
